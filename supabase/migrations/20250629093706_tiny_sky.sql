...
-- RLS Policies with checks to avoid duplicate policy errors
DO $$
BEGIN
    -- Check if the policy exists before creating it for streaming_sessions
    IF NOT EXISTS (
    ...
    -- Check if the policy exists before creating it for streaming_chunks
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
    ...
    -- Check if the policy exists before creating it for streaming_insights
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
