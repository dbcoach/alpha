...
-- RLS Policies with checks to ensure we don't create duplicate policies
DO $$
DECLARE
    policy_exists BOOLEAN;
BEGIN
    -- Check if policy exists for streaming_sessions
    SELECT EXISTS (
        SELECT 1 
        FROM pg_policies 
        WHERE tablename = 'streaming_sessions' 
        AND policyname = 'Users can only access their own streaming sessions'
    ) INTO policy_exists;
    
    -- Create policy only if it doesn't exist
    IF NOT policy_exists THEN
        EXECUTE 'CREATE POLICY "Users can only access their own streaming sessions" ON streaming_sessions
                FOR ALL USING (auth.uid() = user_id)';
    END IF;

    -- Check if policy exists for streaming_chunks
    SELECT EXISTS (
        SELECT 1 
        FROM pg_policies 
        WHERE tablename = 'streaming_chunks' 
        AND policyname = 'Users can only access chunks from their own sessions'
    ) INTO policy_exists;
    
    -- Create policy only if it doesn't exist
    IF NOT policy_exists THEN
        EXECUTE 'CREATE POLICY "Users can only access chunks from their own sessions" ON streaming_chunks
                FOR ALL USING (
                    EXISTS (
    ...
    -- Check if policy exists for streaming_insights
    SELECT EXISTS (
        SELECT 1 
        FROM pg_policies 
        WHERE tablename = 'streaming_insights' 
        AND policyname = 'Users can only access insights from their own sessions'
    ) INTO policy_exists;
    
    -- Create policy only if it doesn't exist
    IF NOT policy_exists THEN
        EXECUTE 'CREATE POLICY "Users can only access insights from their own sessions" ON streaming_insights
                FOR ALL USING (
                    EXISTS (
                        SELECT 1 FROM streaming_sessions 
